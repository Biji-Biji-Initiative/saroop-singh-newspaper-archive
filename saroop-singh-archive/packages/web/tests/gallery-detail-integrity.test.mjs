...
test("published database photographs use their original as the public authority", () => {
  const loader = read("lib/public-gallery-record.ts");
...
test("only the approved, published derivative is exposed for comparison", () => {
  const loader = read("lib/public-gallery-record.ts");
  assert.match(loader, /eq\(restorationRuns\.outputKey, image\.publishedKey\)/);
...
test("gallery details support both legacy and reviewed database records", () => {
  const page = read("app/gallery/[id]/page.tsx");
  assert.match(page, /getPublicGalleryRecord/);
  assert.doesNotMatch(page, /getLegacyCollection\(id\)/);
