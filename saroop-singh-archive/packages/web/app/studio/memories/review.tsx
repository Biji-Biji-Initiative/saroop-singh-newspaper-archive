...
export function MemoryReview() {
  ...
  async function changePublicIdentity(item: Memory) {
    const isPublic = item.publicIdentity?.status === "published";
    const name = item.publicIdentity?.name || item.proposedName;
    if (!name) return;
    if (!window.confirm(`${isPublic ? "Remove" : "Make"} “${name}” ${isPublic ? "private again" : "visible on the public photograph"}?`)) return;
    setBusy(item.id);
    try {
      const response = await fetch(`/api/studio/memories/${item.id}/public-identity`, {
        method: isPublic ? "DELETE" : "POST",
      });
      if (!response.ok) throw new Error("Public identity decision failed");
      const [memories] = await requestAll();
      setItems(memories);
    } catch {
      setError(true);
    } finally {
