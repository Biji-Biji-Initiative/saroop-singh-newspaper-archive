      ...
      {selected && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 sm:p-8" role="dialog" aria-modal="true" aria-label={selected.title} onClick={closeViewer}>
          <div ref={viewerRef} className="relative grid h-screen h-[100dvh] min-h-0 w-full grid-rows-[minmax(20rem,56dvh)_minmax(0,1fr)] overflow-hidden bg-[#f6f1e8] shadow-2xl sm:h-[min(92dvh,56rem)] sm:max-w-6xl sm:rounded-2xl lg:grid-cols-[1.35fr_0.65fr] lg:grid-rows-1" onClick={event => event.stopPropagation()}>
            ...
            <div className="flex min-h-0 flex-col bg-neutral-950 p-3 sm:p-5">
              <div className={`relative min-h-0 flex-1 overflow-hidden rounded-[1.5rem] bg-black ${showComparison ? 'flex items-center overflow-y-auto p-1 sm:p-3' : ''}`}>
                {showComparison && selectedAsset?.kind === 'generation' && selectedAsset.url ? <><RestorationCompare originalUrl={selected.originalUrl} studyUrl={selectedAsset.url} title={selected.title} studyLabel={selectedAsset.label} className="mx-auto w-full max-w-4xl text-white" /><button type="button" onClick={() => setShowComparison(false)} className="absolute bottom-4 right-4 min-h-11 rounded-full bg-white px-4 text-sm font-semibold text-[#17241d] shadow-lg focus:outline-none focus:ring-4 focus:ring-amber-300">Back to featured variation</button></> : <><Image src={selectedAsset?.url || selected.originalUrl} alt={`${selected.title} — ${selectedAsset?.label || 'preserved source'}`} fill unoptimized sizes="(max-width: 1024px) 100vw, 70vw" className="object-contain p-2 sm:p-4" /><span className={`absolute bottom-3 left-3 rounded-full px-3 py-1.5 text-[11px] font-bold uppercase tracking-[.12em] ${selectedAsset?.kind === 'generation' ? 'bg-amber-300 text-[#17241d]' : 'bg-emerald-950/90 text-white'}`}>{selectedAsset?.kind === 'generation' ? selectedAsset.label : 'Fit to screen · complete source'}</span>{selectedAsset?.kind === 'generation' && <button type="button" onClick={() => setShowComparison(true)} className="absolute bottom-3 right-3 flex min-h-11 items-center rounded-full bg-white px-4 text-sm font-semibold text-[#17241d] shadow-lg focus:outline-none focus:ring-4 focus:ring-amber-300">Open split slider</button>}</>}
              </div>
              <div className="mt-4 px-1">
                <p className="text-xs font-semibold uppercase tracking-[.16em] text-stone-300">All images and variations</p>
                <div role="list" aria-label="Source and published image variations" className="mt-3 flex snap-x gap-3 overflow-x-auto pb-2">
