...
export async function GET() {
  ...
  const [images, runs, events, memories, identityTags, identityTagEvents] = await Promise.all([
    ...
    getDb().select().from(publicIdentityTags).orderBy(desc(publicIdentityTags.createdAt)),
    getDb().select().from(publicIdentityTagEvents).orderBy(desc(publicIdentityTagEvents.createdAt)),
  ]);
  const payload = {
    schemaVersion: 3,
    ...
    studio: {
      ...
      publicIdentityTags: identityTags,
      publicIdentityTagEvents: identityTagEvents.map(event => ({ ...event, detail: JSON.parse(event.detail) })),
    },
  };
