...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (provider === "openai") {
      ...
      const response = await fetch("https://generativelanguage.googleapis.com/v1beta/interactions", { method: "POST", headers: { "x-goog-api-key": key, "content-type": "application/json" }, body: JSON.stringify({ model: modelConfig.apiModel, store: false, input: [{ type: "text", text: prompt }, { type: "image", data: bytesToBase64(sourceBytes), mime_type: image.originalType }], response_format: { type: "image", mime_type: "image/png", image_size: "2K" } }), signal: AbortSignal.timeout(110_000) });
