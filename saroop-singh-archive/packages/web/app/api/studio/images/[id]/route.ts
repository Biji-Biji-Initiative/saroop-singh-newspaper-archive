...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const event = getDb().insert(archiveEvents).values({ id: crypto.randomUUID(), imageId: id, eventType: consentChange ? `ai-consent:${requestedAiConsent}` : status !== existing.status ? `status:${existing.status}->${status}` : "metadata:updated", actor: user.email, detail: JSON.stringify(consentChange ? { previous: existing.aiProcessingConsent, decision: requestedAiConsent, wordingVersion: FAMILY_AI_CONSENT_VERSION, evidence: consentEvidence, publicStudyRevertedToSource: Boolean(revokePublishedStudy) } : { changedFields: Object.keys(body).filter(key => key !== "status"), status }) });
  if (requestedAiConsent === "revoked") {
    await getDb().batch([
      getDb().update(archiveImages).set(values).where(eq(archiveImages.id, id)),
      getDb().update(restorationRuns).set({ reviewStatus: "revoked", publishedAt: null }).where(and(eq(restorationRuns.imageId, id), eq(restorationRuns.reviewStatus, "approved"))),
      event,
    ]);
  } else {
    await getDb().batch([
