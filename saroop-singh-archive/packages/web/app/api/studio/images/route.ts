...
export async function GET() {
  ...
  const stored = images.map(image => ({ ...image, readOnlyLegacy: false, tags: JSON.parse(image.tags), originalUrl: `/api/media/${encodeURIComponent(image.originalKey)}`, publishedUrl: image.publishedKey ? `/api/media/${encodeURIComponent(image.publishedKey)}` : null, runs: runs.filter(run => run.imageId === image.id).map(run => ({ ...run, outputUrl: run.outputKey ? `/api/media/${encodeURIComponent(run.outputKey)}` : null })) }));
