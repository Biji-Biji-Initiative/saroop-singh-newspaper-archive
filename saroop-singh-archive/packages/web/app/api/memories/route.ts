...
export async function POST(request: Request) {
  ...
  const day = new Date().toISOString().slice(0, 10); const submissionSourceHash = await sha256Hex(new TextEncoder().encode(`${day}:${forwarded}:memory`));
  const [{ value: submittedToday }] = await getDb().select({ value: count() }).from(memorySubmissions).where(and(eq(memorySubmissions.submissionSourceHash, submissionSourceHash), gte(memorySubmissions.createdAt, `${day} 00:00:00`)));
  if (submittedToday >= 100) return Response.json({ error: "This network has reached today’s memory safety limit. Please contact the archive if your family event is still in progress." }, { status: 429 });
