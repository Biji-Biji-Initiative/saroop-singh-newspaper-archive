...
export function FamilyStudyMaker({
  ...
  if (access !== "active") {
    return <section className="rounded-2xl border border-emerald-900/15 bg-emerald-50 p-4 text-emerald-950" aria-label="Family image workspace">
      ...
      <h3 className="mt-2 font-serif text-2xl leading-tight">Make new versions together.</h3>
      <p className="mt-2 text-sm leading-5 text-emerald-950/75">Your shared family link opens this workspace once—no account and no password. It keeps new versions private while the family compares them.</p>
    </section>;
  }
  ...
  return <section className="rounded-2xl border border-emerald-900/15 bg-emerald-50 p-4 text-emerald-950" aria-labelledby="make-version-heading">
    ...
    <p className="mt-2 text-sm leading-5 text-emerald-950/75">Start from the preserved source, choose a look, and compare the result here. It will not appear publicly unless the family later approves it.</p>

    <div className="mt-4 grid gap-2" role="radiogroup" aria-label="Choose the look for this new image">
      {choices.map(choice => <button key={choice.id} type="button" role="radio" aria-checked={recipe === choice.id} onClick={() => setRecipe(choice.id)} className={`flex min-h-12 items-center justify-between rounded-xl border px-3 text-left transition focus:outline-none focus:ring-4 focus:ring-amber-300 ${recipe === choice.id ? "border-emerald-800 bg-white shadow-sm" : "border-transparent bg-white/60 hover:border-emerald-800/30"}`}><span className="font-semibold">{choice.label}</span><span className="text-xs text-emerald-950/65">{choice.note}</span></button>)}
    </div>
